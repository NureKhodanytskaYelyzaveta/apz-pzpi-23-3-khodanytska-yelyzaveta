...
from models import Base, User, Book, Loan, Reservation
from models import UserRole, BookStatus, ReservationStatus
import hashlib
...
def generate_reservation_otp(reservation: Reservation) -> str:
    ...
    """
    # Використовуємо reservation_id для унікальності + поточну годину для обмеження строку дії
...
def iot_unlock_locker():
    ...
    # Перевіряємо всі активні резервування
...
def iot_confirm_pickup():
    ...
    # Перевіряємо, чи є активне бронювання
    res = db.query(Reservation).filter(
    ...
    # Використовуємо існуючу бізнес-логіку бібліотекаря
    try:
...
def iot_return_by_book():
    ...
    # Знаходимо активну позику за book_id
