...
def create_reservation(db: Session, user_id: int, book_id: int, days: int = 7):
    # 1. Імпортуємо статуси, щоб перевіряти їх
    ...
    # 2. 🔧 ФІКС: Додаємо перевірку статусу ACTIVE
    # Раніше тут перевірялося лише expiry_date, тому воно бачило скасовані броні
    active_res = db.query(Reservation).filter(
        Reservation.book_id == book_id,
        Reservation.status == ReservationStatus.ACTIVE, # <--- ДОДАЙ ЦЕ
...
def get_user_active_reservations(db: Session, user_id: int):
    ...
    return db.query(Reservation).filter(
        ...
        Reservation.status == ReservationStatus.ACTIVE,  # ✅ ВАЖЛИВО: тільки активні
        Reservation.expiry_date > datetime.utcnow()       # ✅ І ще не прострочені
    ).all()
